...
fit_rich_gams = function(climate_data, modularity_data, richness, n_cores = 2){
  library(dplyr)
  library(mgcv)
  library(visreg)
  library(gstat)
  ...
  gam.dat <- climate_data %>%
    ...
    dplyr::bind_cols(data.frame(richness = richness))
  ...
  var.mod.raw <- variogram(richness ~ Longitude+Latitude, data = data.var)
  ...
  mod.gam <- mgcv::bam(richness ~ s(Summer_SST, k = 50) + 
                         s(Winter_SST, k = 50) + 
  ...
  mod.gam.gp <- mgcv::bam(richness ~ s(Summer_SST, k = 50) + 
                            s(Winter_SST, k = 50) + 
  ...
  return(list(gam_spatial = mod.best,
              gam_summary = mod.summary,
              gam_concurvity = mod.concurv,
              gam_effects = mod.partials,
              gam_model = best.mod.gam,
